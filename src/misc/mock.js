...
export function getFakeSailing(sailingDate, transitTime) {
	...
	return {
		...
		deliveryDate: sailingDate.clone().add(transitTime + 2, 'days').toISOString(),
		transhipment: 'Direct',
		routeDetails: {
			...
			portOpenDate: sailingDate.clone().subtract(4, 'days').toISOString(),
			ensCutoffDateTime: sailingDate.clone().subtract(3, 'days').toISOString(),
			docCutoffDateTime: sailingDate.clone().subtract(2, 'days').toISOString(),
			portCutoffDateTime: sailingDate.clone().subtract(1, 'days').toISOString(),
			vesselArrivalDate: sailingDate.clone().add(transitTime, 'days').toISOString()
		},
		costDetails: {
			totalCost: 53225,
			...
			notes: [
				{
					type: 'free_days',
					message: 'Detention free days subject to approval from shipping line.'
				},
				{
					type: 'terminal',
				...
				{
					type: 'currency',
					message: 'Currency exchange used is 1 USD = 71 INR. Final rate subject to fluctuations in currency exchange.'
				}
			]
		}
	}
	...
	return Array.from(
		...
		() => {
			let earliest = moment().add(3, 'days'),
				...
				reference = getRandomDate(
					earliest,
					latest
				);
			return getFakeSailing(
				reference,
				getRandomInt(5, 15)
			);
		}
	).sort(
		({travelDate: a}, {travelDate: b}) => {
			let am = moment(a), bm = moment(b);
			if (am.isAfter(bm)) {
				return 1;
			}
			if (am.isBefore(bm)) {
